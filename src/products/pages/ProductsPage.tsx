      ...
      <div className="grid grid-cols-12 justify-around items-center">
        {
          currentProducts.length > 0 ? (
            ...
            <p className="col-span-12 text-center text-blue-500">Seleccione un indice correcto</p>
          )
        }
      </div>
      <div className="hidden items-center justify-between sm:flex my-2 shadow-xl p-2 rounded-lg" aria-label="Pagination">
        <button 
              ...
              <li key={item} className="text-sm">
                {
                  item === "..." ? (
                    ...
                    <button 
                      ...
                      aria-current={currentPage === Number(item) ? "page" : false} 
                      className={`px-3 py-2 rounded-lg duration-150 hover:text-indigo-600 hover:bg-indigo-50 ${currentPage === Number(item) ? "bg-indigo-50 text-indigo-600 font-medium" : ""}`}
                    >
                      {item}
                    </button>
                  )
